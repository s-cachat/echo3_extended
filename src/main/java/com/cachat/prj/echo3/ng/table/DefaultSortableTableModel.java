 ...
 * <p>
 * <code>DefaultSortableTableModel</code> does not store or copy the data in its
 * <code>TableModel</code>; instead it maintains a map from the row indexes of
 * the view to the row indexes of the model.
 * <p>
 ...
 * <p>
 * This way, the <code>DefaultSortableTableModel</code> appears to hold another
 * copy of the table with the rows in a different order.
 * <p>
 * This also makes this decorator <code>TableModel</code> very light weight and
 * if no sorting is specified then no extra sorting information is kept.
 * <p>
...
public class DefaultSortableTableModel extends AbstractTableModel implements SortableTableModel {
    ...
    private final TableModelListener tableModelListener = new TableModelHandler();
     ...
     *
     * @param underlyingTableModel - the underlying and unsorted
     ...
     * @param underlyingTableModel - the underlying and unsorted
     * <code>TableModel</code>
     * @param columnModel - the <code>TableColumnModel</code> to use
     */
     ...
     *
     * @param newTableModel - the new underlying <code>TableModel</code>
     */
    ...
    public int toUnsortedModelRowIndex(int viewRowIndex) {
        if (viewRowIndex == -1) {
            ...
            SortedRow viewToModelTmp[] = getViewToModelSortedRows();
            if (viewToModelTmp != null && viewRowIndex < viewToModelTmp.length) {
                return viewToModelTmp[viewRowIndex].modelIndex;
            }
            return viewRowIndex;
        }
    }
    ...
    public int toSortedViewRowIndex(int modelRowIndex) {
        int modelToViewTmp[] = getModelToViewSortedRows();
        if (modelToViewTmp != null && modelRowIndex < modelToViewTmp.length) {
            return modelToViewTmp[modelRowIndex];
        }
    ...
    public Object getValueAt(int column, int row) {
        ...
        int newRow = toUnsortedModelRowIndex(row);
        return underlyingTableModel.getValueAt(column, newRow);
