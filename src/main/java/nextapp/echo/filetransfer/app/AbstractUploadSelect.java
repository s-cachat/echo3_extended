...
public abstract class AbstractUploadSelect extends Component {
    ...
    private final UploadProcessListener uploadProcessListener = new UploadProcessListener() {
        ...
        public void uploadComplete(UploadProcessEvent e) {
            DoUploadCompleteLater ducl = new DoUploadCompleteLater(e);
            getApplicationInstance().enqueueTask(ducl.handle, ducl);
        }
        ...
        class DoUploadCompleteLater implements Runnable {

            public final TaskQueueHandle handle;
            private final UploadProcessEvent e;

            public DoUploadCompleteLater(UploadProcessEvent e) {
                this.handle = AbstractUploadSelect.this.getApplicationInstance().createTaskQueue();
                this.e = e;
            }

            @Override
            public void run() {
                doUploadComplete(e.getUpload());
                ApplicationInstance.getActive().removeTaskQueue(handle);
            }
        }
    };
    ...
    /**
     * Adds an <code>UploadProgressListener</code> to be notified of file upload
     * progress.
     * <p>
     * <strong>WARNING</strong>: Upload progress events are NOT fired from
     * within user interface threads. Thus implementations MAY NOT update the
     * state of the application's user interface or component hierarchy. An
     * {@link UploadListener} should instead be used for such purposes.
     *
     * @param l the listener to add
     */
     ...
     */
    public void cancel() {
        UploadProcess localUploadProcess = this.uploadProcess;
        if (localUploadProcess != null) {
            localUploadProcess.cancel();
        }
    }
    ...
    /**
     * Determines if any <code>UploadProgressListener</code>s are currently
     * registered.
     *
...
//                doUploadComplete(uploads[i]);
    ...
    /**
     * Removes an <code>UploadProgressListener</code> to be notified of file
     * upload progress.
     * <p>
     * <strong>WARNING</strong>: Upload progress events are NOT fired from
     * within user interface threads. Thus implementations MAY NOT update the
    ...
    /**
     * Removes an <code>UploadListener</code> from being notified of file
     * uploads.
     *
     * @param l the listener to add
     */
    ...
    /**
     * Sets the active {@link UploadProcess} representing incoming uploads. This
     * operation does not and may not change the UI state, it may be invoked
     * outside of a user interface thread.
     *
