...
/**
 * A representation of an upload process. Multiple {@link Upload} objects may be
 * contained in a single <code>UploadProcess</code> if they are being
 * transferred from client to server at the same time.
 */
public class UploadProcess {
    ...
    /**
     * Array cache of uploads. Cleared when uploads changed, lazy (re)created by
     * invocations to {@link #getUploads}
     */
    ...
    /**
     * Adds an {@link UploadProcessListener} to receive notification of
     * {@link UploadProcessEvent}s.
     *
    ...
    /**
     * Cancels the <code>UploadProcess</code>. All contained uploads will be
     * canceled. {@link UploadProcessListener}s will be notified of the
     * cancellation.
     */
    public void cancel() {
    ...
    /**
     * Completes an individual {@link Upload}. The specified {@link InputStream}
     * and size information will be stored in the {@link Upload}.
     *
    ...
    public void complete(Upload upload, InputStream in, long size) {
        logger.severe("UploadReceiverService : UploadProcess.complete size=" + size);
        ((UploadImpl) upload).setSize(size);

        if (upload.getStatus() == Upload.STATUS_IN_PROGRESS) {
            logger.severe("UploadReceiverService : UploadProcess.complete INPROGRESS->COMPLETE");
            ((UploadImpl) upload).setInputStream(in);
            notifyComplete(upload);
        } else {
            logger.severe("UploadReceiverService : UploadProcess.complete NOT INPROGRESS");
            if (in != null) {
                try {
                    in.close();
    ...
    /**
     * Completes an individual {@link Upload}. The specified {@link File} and
     * size information will be stored in the {@link Upload}.
     *
     * @param upload the <code>Upload</code>
    ...
    /**
     * Determines if all uploads have been completed, i.e., no uploads have a
     * status of {@link Upload#STATUS_IN_PROGRESS}
     *
    ...
    private void notifyComplete(Upload upload) {
        logger.severe("UploadReceiverService : UploadProcess.notifyComplete");
        ...
        listeners.toArray(listenerArray);
        for (int i = 0; i < listenerArray.length; ++i) {
            logger.severe("UploadReceiverService : UploadProcess.notifyComplete " + i + " : " + listenerArray[i].getClass().getName() + " : " + listenerArray[i]);
            listenerArray[i].uploadComplete(e);
        }
        logger.severe("UploadReceiverService : UploadProcess.notifyComplete DONE");
    }

    /**
     * Sets the progress of a specific {@link Upload}. Reports upload progress
    ...
    /**
     * Removes an {@link UploadProcessListener} from receiving notification of
     * {@link UploadProcessEvent}s.
     *
