console.log("loaded datetimerange.js");
/* global moment */
...
(function () {
    DateRangePicker = function (element, options, cb) {
        ...
        if (typeof options.template !== 'string')
            options.template =
                    ...
                    '<div class="daterangepicker-modal"></div>' +
        ...
        } else if (typeof options.startDate === 'object') {
        ...
        } else if (typeof options.endDate === 'object') {
        ...
        if (this.startDate && this.minDate && this.startDate.isBefore(this.minDate))
            this.startDate = this.minDate.clone();

        // sanity check for bad options
        if (this.endDate && this.maxDate && this.endDate.isAfter(this.maxDate))
        ...
        let clearBtnEl = this.container.querySelector('.clearBtn');
        jq.addClass(applyBtnEl, this.buttonClasses);
        jq.addClass(cancelBtnEl, this.buttonClasses);
        jq.addClass(clearBtnEl, this.buttonClasses);
        ...
        if (this.clearButtonClasses.length)
            jq.addClass(clearBtnEl, this.clearButtonClasses);
        jq.html(applyBtnEl, this.locale.applyLabel);
        jq.html(cancelBtnEl, this.locale.cancelLabel);
        jq.html(clearBtnEl, this.locale.clearLabel);
        ...
        this.clickClearProxy = function (e) {
            this.clickClear(e);
        }.bind(this);
        jq.on(drpButtonsEl, 'click', 'button.clearBtn', this.clickClearProxy);
    ...
    DateRangePicker.prototype = {
        ...
        setStartDate: function (startDate) {
            ...
            jq.html(this.container.querySelector('.drp-selected'), this.getFormattedText());
        ...
        getFormattedText: function () {
            ...
            var sep = "";
            if (this.startDate !== null) {
                text += this.startDate.format(this.locale.format);
                sep = this.locale.separator;
            }

            if (!this.singleDatePicker && this.endDate !== null) {
                text += sep + this.endDate.format(this.locale.format);
            }
            return text;
        },
        ...
        updateMonthsInView: function () {
            if (this.endDate) {
                ...
                if (this.startDate && this.startDate.isValid() && this.leftCalendar.month.format('YYYY-MM') != this.startDate.format('YYYY-MM') && this.rightCalendar.month.format('YYYY-MM') != this.startDate.format('YYYY-MM')) {
                    this.leftCalendar.month = this.startDate.clone().date(2);
                    this.rightCalendar.month = this.startDate.clone().date(2).add(1, 'month');
                } else {
                    this.leftCalendar.month = moment().date(2);
        ...
        renderTimePicker: function (side) {
            if (side == 'left' && (!this.startDate || !this.startDate.isValid()))
                return;
            // Don't bother updating the time picker if it's currently disabled
            // because an end date hasn't been clicked yet
            if (side == 'right' && (!this.endDate || !this.endDate.isValid()))
                return;
            ...
            if (side == 'left') {
                ...
                if (selected && maxDate && selected.isAfter(maxDate))
        ...
        move: function () {
            let picker = this.container.querySelector('.daterangepicker');
            let vh = Math.max(document.documentElement.clientHeight || 0, window.innerHeight || 0);
            picker.style.top = vh / 2 - picker.offsetHeight / 2 + 'px';

            // Original code, non modal
...
//
//            if (this.opens == 'left') {
//                // FIX absolute positioning, before :
//                // var containerRight = parentRightEdge - elementOffset.left - this.element.offsetWidth;
//                var containerRight = parentRightEdge - this.element.offsetWidth;
...
//                var containerLeft = elementOffset.left + this.element.offsetWidth / 2 - containerWidth / 2;
//                if (containerLeft < 0) {
//                    this.container.style.top = containerTop + 'px';
//                    this.container.style.right = 'auto';
//                    this.container.style.left = '9px';
...
//                var containerLeft = elementOffset.left;
...
//            }
        ...
        show: function (e) {
            ...
            let modal = this.container.querySelector('.daterangepicker-modal');
            document.addEventListener('mousedown', this._outsideClickProxy);
            modal.addEventListener('mousedown', this._outsideClickProxy);
            // also support mobile devices
            document.addEventListener('touchend', this._outsideClickProxy);
            modal.addEventListener('touchend', this._outsideClickProxy);
            jq.on(document, 'click', '[data-toggle=dropdown]', this._outsideClickProxy);
            jq.on(modal, 'click', '[data-toggle=dropdown]', this._outsideClickProxy);
            // and also close when focus changes to outside the picker (eg. tabbing between controls)
            document.addEventListener('focusin', this._outsideClickProxy);
            modal.addEventListener('focusin', this._outsideClickProxy);
            ...
            this.oldStartDate = this.startDate && this.startDate.isValid() ? this.startDate.clone() : null;
            this.oldEndDate = this.endDate && this.endDate.isValid() ? this.endDate.clone() : null;
            this.previousRightTime = this.endDate && this.endDate.isValid() ? this.endDate.clone() : null;
...
//            }
            ...
            if ((!this.startDate && this.oldStartDate)
                ...
                this.callback(!this.startDate ? null : this.startDate.clone(), this.endDate === null ? null : this.endDate.clone(), this.chosenLabel);
...
//            this.updateElement();
            ...
            if (this._outsideClickProxy) {
                ...
                modal.removeEventListener('mousedown', this._outsideClickProxy);
                modal.removeEventListener('touchend', this._outsideClickProxy);
                jq.off(modal, 'click', '[data-toggle=dropdown]', this._outsideClickProxy);
                modal.removeEventListener('focusin', this._outsideClickProxy);
            }
        ...
        outsideClick: function (e) {
            ...
            if (
                    ...
                    target.closest(jq.getSelectorFromElement(this.container.querySelector('.daterangepicker'))) ||
        ...
        hoverDate: function (e) {
            ...
            if (!this.endDate) {
                ...
                for (let i = 0; i < tdElList.length; ++i) {
                    ...
                    if (tdElList[i].classList.contains('week'))
                        return;
        ...
        clickClear: function (e) {
            ...
            this.hide();
            e.target.dispatchEvent(new CustomEvent('clear.daterangepicker', {bubbles: true, detail: this}));
        },
        ...
        elementChanged: function (e) {
            ...
            // Only set values if 'TAB' or 'ENTER' are pressed
            if (e && ((e.keyCode === 9) || (e.keyCode === 13))) {
                ...
                if (!input || !input.length) {
                    ...
                    dateString = input.split(this.locale.separator);
                }
                ...
                if (this.singleDatePicker || end === null) {
                    end = start !== null ? start.clone() : null;
                }
        ...
        updateElement: function () {
            if (this.element.tagName === 'INPUT' && this.autoUpdateInput) {
                this.element.value = this.getFormattedText();
            }
        },
        ...
        remove: function () {
            ...
            if (this.clickClearProxy) {
                jq.off(drpButtonsEl, 'click', 'button.clearBtn', this.clickClearProxy);
                delete this.clickClearProxy;
            }
            if (this.element.tagName === 'INPUT' || this.element.tagName === 'BUTTON') {
                if (this.showProxy) {
