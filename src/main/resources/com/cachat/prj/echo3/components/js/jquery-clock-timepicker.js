...
(function ($) {
    ...
    $.fn.clockTimePicker = function (options, _value) {
        ...
        return this.each(function () {
            ...
            if (isMobile()) {
                ...
                okButton.on('click', function () {
                    if (isMobile())
                        element.val(getInputElementValue());
                    if (settings.vibrate)
             ...
             ************************************************************************************************/
            function blurAll() {
                if (document.activeElement != inputElement.get(0))
                    return;
            ...
            function processMouseWheelEvent(event) {
                ...
                event.preventDefault();
                if (new Date().getTime() - lastMouseWheelTimestamp < 100)
                    return;
            ...
            function repaintClockHourCanvas(hoverHour, hoverSign) {
                ...
                if (!isMobile() && hoverHour) {
                    ...
                    if (settings.maximum && !isTimeSmallerOrEquals((negative ? '-' : '') + (hoverHour == 24 ? '00' : hoverHour) + ':00', settings.maximum))
                        isMinMaxFullfilled = false;
                    if (settings.minimum && !isTimeSmallerOrEquals(settings.minimum, (negative ? '-' : '') + (hoverHour == 24 ? '00' : hoverHour) + ':00', true))
                        isMinMaxFullfilled = false;
                    if (isMinMaxFullfilled) {
                        ctx.beginPath();
            ...
            function showTimePicker() {
                if (!element.val())
                    ...
                    setInputElementValue(formatTime(element.val()));
            ...
            function onInputElementKeyDown(event) {
                ...
                //COLON OR DOT
                    ...
                    event.preventDefault();
                    if (getInputElementValue().length == 0)
                        setInputElementValue('0');
                    setInputElementValue(formatTime(getInputElementValue()));
                    if (settings.precision != 60) {
                        selectMinuteOnInputElement();
                        if (selectionMode != 'MINUTE')
                ...
                //ARROW UP OR "+" SIGN, ARROW DOWN OR "-" SIGN
                    ...
                    var h = parseInt(RegExp.$2);
                    if (settings.duration && settings.durationNegative && RegExp.$1 == '-')
                        h = -h;
                    var m = RegExp.$4 ? parseInt(RegExp.$4) : 0;
                    if (selectionMode == 'HOUR')
                        ...
                        if (m < 0)
                            ...
                            m = 60 - settings.precision;
                    }
                    var min = settings.minimum;
                    if ((!settings.duration || !settings.durationNegative) && min[0] == '-')
                        min = '0:00';
                    ...
                    if (selectionMode == 'HOUR') {
                        if (!isTimeSmallerOrEquals(newVal, max))
                            newVal = max;
                        else if (!isTimeSmallerOrEquals(min, newVal))
                            newVal = min;
                    }
                    if (oldVal != newVal) {
                        setInputElementValue(formatTime(newVal));
            ...
            function onInputElementKeyUp(event) {
                if (event.shiftKey || event.ctrlKey || event.altKey || !event.key.match(/^[0-9]{1}$/))
                    return;
        ...
        function isTimeSmallerOrEquals(time1, time2, doNotInvolveMinutePart) {
            ...
            if (RegExp.$4 && !doNotInvolveMinutePart)
